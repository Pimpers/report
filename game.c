...
int gameBuffer(char *screenBuf, int width, int height)
{
    ...
    while (tries < maxTries && !found && hp > 0) // 목숨이 0보다 크면 게임 진행
    {
        ...
        {
            found = 1;
            gamewriteStringToBuffer(screenBuf, "[*****You win!*****]", 5, 2, width);
            Sleep(3000);
        }
        if (hp == 0)
        {
                // "Write One Letter" 문자열 지우기
            gamewriteStringToBuffer(screenBuf, "[*****Game Over!*****]", 5, 2, width); // "Game Over!" 출력
            Sleep(3000);
        }
    }
}
...
{
    gameclearBuffer(screenBuf, 30, 15);                                    
    gamewriteStringToBuffer(screenBuf, "[Write One Letter]", 5, 2, width); 
    gamewriteStringToBuffer(screenBuf, "hint:menu ", 4, 4, width);         
    gameBuffer(screenBuf, 30, 15);                                         
}
